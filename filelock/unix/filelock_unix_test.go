...
func (s *FileLockTestSuite) TestConcurrentLocks() {
	...
	// (should fail with ErrLockHeld or ErrAlreadyLocked)
	lock2 := New(lockPath)
	err = lock2.Lock()

	if !errors.Is(err, filelock.ErrLockHeld) && !errors.Is(err, filelock.ErrAlreadyLocked) {
		s.Assert().Failf(
			"Expected ErrLockHeld or ErrAlreadyLocked",
			"got: %v",
			err,
		)
	}
...
func (s *FileLockTestSuite) TestLockWithTimeout() {
	...
	// Try to acquire with a short timeout (should fail with ErrTimeout or ErrAlreadyLocked)
	lock2 := New(lockPath)
	err = lock2.LockWithTimeout(100 * time.Millisecond)
	if err != filelock.ErrTimeout && err != filelock.ErrAlreadyLocked {
		s.Assert().Failf(
			"Expected ErrTimeout or ErrAlreadyLocked",
			"got: %v",
			err,
		)
	}
...
func (s *FileLockTestSuite) TestNonBlockingBehavior() {
	...
	go func() {
		...
		// We expect a timeout error or already locked
		if err != filelock.ErrTimeout && err != filelock.ErrAlreadyLocked {
			s.T().Errorf("Expected ErrTimeout or ErrAlreadyLocked, got: %v", err)
		}
...
// TestUnixSpecificBehavior tests Unix-specific behavior of file locks
func (s *FileLockTestSuite) TestUnixSpecificBehavior() {
	lockPath := filepath.Join(s.tempDir, "unix-specific.lock")
	...
	s.Require().NoError(err, "Lock file should exist")
